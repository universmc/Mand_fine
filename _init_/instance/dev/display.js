...
async function main() {
  ...
  const streamPlayerHeader = `print "╔═════════════════════════════════════════════════════════════════════════════╗"`;
  const streamPlayerMain= `print "║                                                                          /%>║"`;
  const streamPlayerFooter = `print"╚═════════════════════════════════════════════════════════════════════════════╝"`
  
  const streamPlay = `${PS1}+${streamLoader}+${streamAD}+${streamRUN}+${streamAD}+${streamEND}`;
  const streamPlayer = `${streamPlayerHeader}+${streamPlayerMain,streamPlay}+${streamPlayerFooter}`;

  const streamSignal = `${streamPlay}+${streamPlayer}`

  const chatCompletion = await groq.chat.completions.create({
    "messages": [
      {role: "assistant",name:"[📔.codex]", content:"phase[01]:[RUN]:[brainstorming(initalisation.timestamp)]"},
      {role: "assistant",name:"[📔.codex]", content:`${asciiWindow}+${stream}`},
